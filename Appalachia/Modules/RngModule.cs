...
namespace Appalachia.Modules
{
    ...
    public class RngModule : ModuleBase<SocketCommandContext>, IModuleBase
    {
        ...
        [Command("rng"), Alias("random", "rand", "randnum"), Name(Modules.RngModule.Source + "/" + Source)]
        ...
        [Command("roll"), Alias("random", "rand", "randnum"), Name(Source)]
        public async Task BasicRng(int max)
        {
            await BasicRng(1, max);
        }
        [Command("rng"), Alias("random", "rand", "randnum"), Name(Source)]
        public async Task BasicRng()
        {
            await BasicRng(1, 100);
        }

        [Command("roll"), Alias("dice", "die"), Priority(int.MinValue), Name(Modules.RngModule.Source + "/" + Source + "/Dice")]
